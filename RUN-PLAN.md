...
| 9 | C+D | midterm verify **and** build `out/pharmacology-midterm-01.md` | 153 Q | doing | split into two rows if #7–8 land more than ~250 Q |
...
| 13 | C | practice verify | 481 Q (rows 2, 10, 11, 12 combined) | doing | **Splitting into more than the originally-planned 3 sessions** — the first session (this one) covered PHARM-P-001–099 only (Fawzi/Malik's open-tier general PK/PD set, the first of row 2's three source sections): 93 verified, 6 external, 0 conflict, 0 not-in-source, 0 needs-eye. Written to `pharmacology-practice.verify.md` (with a `scope:` field marking it partial) + `pharmacology-practice.report.md`. **Resume at PHARM-P-100** — next up is the rest of row 2's batch (PHARM-P-100–174, claimed tier, Rama/Sana's PK/PD + antimicrobial sets — note PHARM-P-123–129 look off-syllabus/autonomic and will likely be `not-in-source`), then rows 10–11's material (PHARM-P-175–408), then row 12's (PHARM-P-409–481). **A meaningful chunk will not route to `source.md`** — rows 10 and 11 carry material this course never taught (off-syllabus autonomics, opioids, adrenergics, morphine, anaesthesia, insulin, aspirin). That is `not-in-source`, which is a real status, not a failure. Before defaulting to it, try `slides/_reference/Pharmacology___Toxicology_-_3rd_Ed..pdf` as an external authority → `external` tier, the way microbiology's finals batch resolved 7 |
