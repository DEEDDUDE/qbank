...
| 13 | C | practice verify | 481 Q (rows 2, 10, 11, 12 combined) | todo | **A meaningful chunk will not route to `source.md`** — rows 10 and 11 carry material this course never taught (off-syllabus autonomics, opioids, adrenergics, morphine, anaesthesia, insulin, aspirin). That is `not-in-source`, which is a real status, not a failure. Before defaulting to it, try `slides/_reference/Pharmacology___Toxicology_-_3rd_Ed..pdf` as an external authority → `external` tier, the way microbiology's finals batch resolved 7. Likely needs splitting into 2–3 sessions |
