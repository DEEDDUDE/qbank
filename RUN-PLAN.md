...
| 13 | C | practice verify | 481 Q (rows 2, 10, 11, 12 combined) | doing | **Three sessions done, rows 2 and 10 now finished.** Session 1: PHARM-P-001–099 — 93 verified, 6 external. Session 2: PHARM-P-100–174 — 55 verified, 10 external, 10 not-in-source, 0 conflict across 75 claimed questions. Session 3 (this one): PHARM-P-175–270 (row 10's TEST BANK + dental-students sets) — 95 of 96 questions were official tier, passed through untouched at zero cost per Job C's own rule; the lone open-tier exception (PHARM-P-244) resolved cleanly from Ch. 2.3's Phase I oxidation description. Six more graph questions (PHARM-P-177, 179, 180, 181, 186, 187) all confirmed from their figures, answers already known from the key. Combined so far: PHARM-P-001–270, 244 verified · 16 external · 0 conflict · 10 not-in-source · 0 needs-eye. Written to `pharmacology-practice.verify.md` + `.report.md` (both updated in place, `scope:` field still marks this as partial). **Resume at PHARM-P-271** — row 11's Pharma020 material (PHARM-P-271–408), then row 12's (PHARM-P-409–481). **A meaningful chunk will not route to `source.md`** — rows 10 and 11 carry material this course never taught (off-syllabus autonomics, opioids, adrenergics, morphine, anaesthesia, insulin, aspirin). That is `not-in-source`, which is a real status, not a failure. Before defaulting to it, try `slides/_reference/Pharmacology___Toxicology_-_3rd_Ed..pdf` as an external authority → `external` tier, the way microbiology's finals batch resolved 7 |
